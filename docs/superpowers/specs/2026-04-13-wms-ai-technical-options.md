...
- 对当前 ASP.NET Core / YARP 生态融合不如 `YARP`
...
- 微软官方反向代理项目
- 与 ASP.NET Core 集成自然
- 更适合本项目的统一入口、路由、鉴权扩展和观测接入

缺点：

- 需要自己约束配置和 transform 规范
...
- 符合业界“配置外置、密钥外置”的最佳实践
- 配置、特性开关、密钥轮换、审计边界清晰
- 适合 `Aspire + YARP + 多服务` 架构
...
- 配置中心固定选 `Nacos`
- 密钥管理走 `Vault / 云 KMS / External Secrets`
- `Nacos` 负责：
  - 环境级配置
  - 特性开关
  - 动态刷新
...
本项目代码里只保留“配置消费”和“配置快照落库”能力，不实现配置中心本身。
...
- `Platform` 通过管理接口维护模型 provider / profile / routing policy
- `Inbound` 不直接查询 `AiDb` 的模型配置
- `Inbound` 只提交：
  - `scene_code`
  - `tenant_id`
  - `warehouse_id`
  - `business_object_id`
- `AiGateway` 负责解析最终生效的模型配置，并把运行快照绑定到本次 `AiInspectionRun`
- 环境级默认值和开关优先从 `Nacos` 读取
- `AiGateway` 在运行前把 `Nacos + AiDb` 的结果合并成最终生效配置

这意味着：

- `AiDb` 负责 AI 运行时配置真相和审计快照
- 业务服务只依赖 AI 能力接口，不依赖 AI 配置表结构
- 配置中心负责“环境默认值”，`AiDb` 负责“业务可审计配置”
...
- 具备 dashboard
- 对运营和排障更友好
- 对“补偿扫描、超时清理、归档、统计”这类任务已经足够

缺点：

- 对特别复杂的企业级调度表达能力不如 `Quartz.NET`
...
- 面向基础设施调度更自然
- 支持 cron、持久化 job store、集群
- 适合清理、扫描、归档、对账、补偿这类系统任务

缺点：

- 业务操作体验不如 Hangfire 直观
...
- 主业务链不用定时任务驱动
- 主业务链继续用 `MAF Workflow + CAP`
- 定时任务只处理系统维护、补偿扫描和运营统计
- `Hangfire Dashboard` 仅面向运维和研发开放，不作为业务后台页面
