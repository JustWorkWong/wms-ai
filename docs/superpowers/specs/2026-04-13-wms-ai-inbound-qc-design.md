...
- 本次 AI 执行主干：`MAF`，不是 `Semantic Kernel`
- 前端技术栈：`Vue`
- AI 交互协议：`AG-UI`
- 平台形态：`SaaS 混合型`
- 数据范围：`多租户 + 多仓库`
...
- `Aspire AppHost + ServiceDefaults` 作为本地编排与环境装配主线
- `AI Gateway` 的 session、checkpoint、上下文压缩、模型配置、AG-UI 转接
- `MAF Workflow + Skill + MCP + Function Calling` 的 AI 检验流程编排
...
### 1. MAF 是主执行框架，不用 SK 做主链路

这次后端 AI 主线明确为：

- `Microsoft.Extensions.AI`：统一模型调用接口
- `MAF`：Agent 会话、历史、工具、上下文、流式执行运行时
- 项目自定义 `Workflow`：质检流程状态、审批、恢复、审计

`Semantic Kernel` 不作为这次项目的主执行引擎，也不作为规划器或工作流核心。原因很直接：

- 这次系统是稳定业务流程，不是通用 prompt 资产管理平台
- 重点在 `session / checkpoint / workflow state / 审计 / 恢复`
- 这些更适合 `MAF + 自定义业务 workflow` 的组合

### 2. Aspire 是本地编排和后续交付出口

本地开发和联调必须使用 `Aspire`：

- `AppHost` 统一拉起 `Gateway`、`Platform`、`Inbound`、`AI Gateway`
- 统一装配 `PostgreSQL`、`Redis`、`RabbitMQ`、`Object Storage`
- 统一注入连接串、环境变量、健康检查、OpenTelemetry

后续交付要求：

- 通过 `Aspire` 导出或生成 `docker compose`
- 通过 `Aspire` 导出或生成 `k8s yaml`

这意味着 `Aspire` 不只是本地玩具，而是这次环境模型的来源。

### 3. Aspire 必须采集 MAF 全链路日志

`Aspire` 里不能只看 HTTP 和数据库日志，还必须覆盖 `MAF` 运行时关键事件：

- session 创建、恢复、关闭
- workflow 节点进入、退出、失败、重试
- tool/function 调用开始、结束、异常
- MCP 请求与响应摘要
- skill 装载与执行摘要
- checkpoint 创建与恢复
- 模型路由结果、耗时、token 使用、降级

日志原则：

- 业务审计和 AI 运行日志分流
- 默认记录结构化元数据，不默认落完整敏感 prompt 正文
- 必要时通过红线开关采样详细内容，避免生产泄露
...
### 1. 不建议把服务直接命名成一堆 `xxApi`

你提的这个点是对的。`PlatformCore.Api`、`InboundQcCore.Api` 这种名字最多只适合表达“HTTP 宿主”，不适合表达真实架构。
...
也就是说，对外部署的是一个服务，但服务内部不应该只剩一个 `Api` 项目。
...
- `Host`：只负责 HTTP/gRPC/MCP/AG-UI 暴露、DI、配置、中间件
- `Application`：用例编排、DTO、命令查询、事务边界
- `Domain`：领域实体、规则、值对象、领域服务
- `Infrastructure`：EF Core、Redis、MQ、对象存储、配置中心适配

这样命名后，服务名表达“业务边界”，`Host` 才表达“宿主协议”。这比一排 `xxApi` 更稳。

### 3. Aspire 对应关系

`Aspire AppHost` 至少编排以下资源：
...
实体不是平铺罗列，而是沿着“通知 -> 收货 -> 质检 -> AI 建议 -> 正式结论”流动。
...
- `InboundNotice` 是业务意图，表示仓库预期接收入库内容
- `Receipt` 是现场事实，表示实际收到了什么
- `QcPlan` 是系统根据规则生成的检验计划
- `QcTask` 是具体可执行的检验任务
- `EvidenceAsset/EvidenceBinding` 是现场证据
- `AiInspectionRun/AiSuggestion` 是 AI 的辅助判断链
- `QcDecision` 才是最终业务真相

最重要的边界是：

- `AiSuggestion` 不能替代 `QcDecision`
- `Receipt` 不能替代 `InboundNotice`
- `EvidenceAsset` 只保存证据引用，不保存业务结论
...
flowchart TD
    ...
    C --> D["按规则生成 QcPlan / QcTask"]
    D --> E["质检员录入检查项并上传证据"]
    E --> F["触发 AI 检验 Workflow"]
    F --> G{"证据是否充分?"}
    G -- "否" --> H["AI 生成缺失证据清单"]
    H --> E
    G -- "是" --> I["AI 输出结构化建议"]
...
- `ASN` 是业务起点，但不是现场真相
- `Receipt` 把“预期到货”转成“实际收货”
- `QcPlan/QcTask` 把规则转换成具体执行单元
- AI 不是直接判定器，而是证据解释器、建议器、缺失项追问器
- 自动通过只是 `QcDecision` 的一种来源，不是单独的业务体系

## MAF Workflow 与 Skill / MCP / Function Calling 的配合

这次检验流程必须由 `MAF Workflow` 驱动，不是让 Agent 自由发挥整条业务链。

### 1. 四层职责边界

- `Workflow`
  控制流程顺序、状态落库、审批、恢复、重试
- `Agent`
  在单个检验节点内做动态判断
- `Skill`
  规定这类检验任务的标准方法
...
- `Skill` 给方法
- `MCP` 给外部能力
- `Function Calling` 给本地确定性动作

### 2. 为什么检验主线要用 Workflow
...
所以主链必须是 workflow，而不是一段大 prompt。

### 3. Workflow 建议节点

建议把单个 `QcTask` 的 AI 检验流程切成下面这些 durable 节点：
...
10. `WaitManualReview`（仅异常分支）
11. `FinalizeDecision`

### 4. Skill / MCP / Function Calling 在流程中的位置

#### Skill

Skill 用来规定检验方法，不直接连业务系统。至少应存在几类 skill：
...
也就是说，MCP 更像 AI 的外部工具插座。

#### Function Calling

Function Calling 用于服务内确定性动作，适合放：
